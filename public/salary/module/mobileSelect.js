	...
	MobileSelect.prototype = {
			...
			for (var i = 0; i < wheelsData.length; i++) {
				...
				if (_this.jsonType) {
					for (var j = 0; j < wheelsData[i].data.length; j++) {
						//行
						tempHTML += '<li data-id="' + wheelsData[i].data[j][_this.keyMap.id] + '">' +
		...
		setCurDistance: function (indexArr) {
			...
			for (var i = 0; i < _this.slider.length; i++) {
				...
				// init selected item's className
				_this.slider[i].children[indexArr[i]].classList.add('selected')
			}
			_this.curDistance = temp
		},

		setCurSelectItems: function(index) {
			var children = this.slider[index].children
			for (var j = 0; j < children.length; j++) {
				children[j].classList.remove('cur-item')
			}
			// 设置 className
			if (this.offsetSum > 0) {
				console.log('up')
				for (var j = 0; j < this.curSelectItems.length; j++) {
					this.curSelectItems[j] = this.curSelectItems[j].previousSibling || this.curSelectItems[j]
					this.curSelectItems[j].classList.add('cur-item')
				}
			} else {
				console.log('down')
				for (var j = 0; j < this.curSelectItems.length; j++) {
					this.curSelectItems[j] = this.curSelectItems[j].nextSibling || this.curSelectItems[j]
					this.curSelectItems[j].classList.add('cur-item')
				}
			}				
		},
		...
		getInnerHtml: function (sliderIndex) {
			...
			var li = _this.slider[sliderIndex].getElementsByTagName('li')[index]

			return li ? li.innerHTML : null
		},

		setSelectedItemColor(theSlider, index) {
			var liNodes = theSlider.children
			for (var i = 0; i < liNodes.length; i++) {
				liNodes[i].classList.remove('selected')
			}
			var selectedIndex = parseInt(-((this.curDistance[index] / this.liHeight ) - 1))
			liNodes[selectedIndex].classList.add('selected')
		},
			...
			switch (event.type) {
				...
				case 'touchend':
					...
					if (_this.offsetSum == 0) {
						...
						_this.setSelectedItemColor(theSlider, index)
